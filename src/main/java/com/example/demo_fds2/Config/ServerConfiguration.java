package com.example.demo_fds2.Config;
...
import com.example.demo_fds2.TransactionEnginee.TransDataAttribute.TransDataAttributeService;
...
public class ServerConfiguration {
    protected final NetworkCfgService networkService;
    protected final EndpointService endpointService;
    protected final TransDataAttributeService transDataAttributeService;

    @Autowired
    public ServerConfiguration(NetworkCfgService networkService, EndpointService endpointService, TransDataAttributeService transDataAttributeService) {
            ...
            HttpServer.create()
                    ...
                    .route(routes -> {
                        for (Endpoint endpoint : endpoints) {
                            routes.post(endpoint.getUrl(), this::handle);
                        }
                    })
                    .bindNow();
        }
                    ...
                    if (this.isEndpointAllowed(uri, attr.getEndpoint().getUrl())) {
                        ...
                        if (this.hasChildAttributes(attr.getAttrId(), dataAttr)) {
                            if (fieldValue.isObject()){
                                ...
                                log.info(fullFieldName +" is an object");
                            } else if (fieldValue.isArray()) {
                                this.processJsonArray(fieldValue, dataAttr, uri, fullFieldName, storeAttr, errorMessage);
                                log.info(fullFieldName +" is an array");
                            } else {
                                errorMessage
                                        .append("Field '")
                                        .append(fullFieldName).append("' with child attributes must be an object or array.");
                            }
                        } else {
    ...
    private void processJsonArray(JsonNode node, List<TransDataAttribute> dataAttr, String uri, String parentFieldName, Map<String, Object> storeAttr, StringBuilder errorMessage) {
        for (int i = 0; i < node.size(); i++) {
            ...
            if (arrayItem.isObject()) {
                ...
                log.info("Array item is object: " + parentFieldName + "[" + i + "]");
            } else {
